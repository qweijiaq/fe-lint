...
`wj-fe-lint` 是[GopherLint 前端编码规范工程化](https://wj-fe.github.io/gopher-lint/)的配套 Lint 工具，可以为项目一键接入规范、一键扫描和修复规范问题，保障项目的编码规范和代码质量。
...
`wj-fe-lint` 会读取执行目录下的 `wj-fe-lint.config.js` 作为配置文件。`wj-fe-lint init` 会在执行目录下新增如下的 `wj-fe-lint.config.js` 文件：
